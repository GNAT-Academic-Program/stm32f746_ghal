package body STM32F746_USART is
   ...
   begin
      return (Periph => STM32F746_USART.Periph);
   end Make_Device;

   procedure Compute_BRR
     (Pclk     : Natural;
      Baud     : Natural;
      Mantissa : out STM32F746.UInt12;
      Fraction : out STM32F746.UInt4)
   is
   ...
   begin
      ...
      Mantissa :=
        (if Mant > Natural (STM32F746.UInt12'Last)
         then STM32F746.UInt12'Last
         else STM32F746.UInt12 (Mant));

      Fraction :=
        (if Frac > Natural (STM32F746.UInt4'Last)
         then STM32F746.UInt4'Last
         else STM32F746.UInt4 (Frac));
   end Compute_BRR;
   ...
   is
      ...
      Mant : STM32F746.UInt12;
      Frac : STM32F746.UInt4;
   begin
      --  On the F746 the USART kernel clock is the APB bus clock (the
      --  reset default of RCC_DKCFGR2; our Clock_Tree assumes that
      --  default). Unlike the G4 body, we do NOT force HSI here -- there
      --  is no HSIKERON on the F746 RCC.CR, and the USART does not run
      --  off HSI. The bus clock comes from Get_Clock below.
      RCC_Enable;
      ...
      Dev.Periph.CR2.STOP :=
        (if Cfg.Stop_Bits = Usart_Types.Stop_2
         then STM32F746.UInt2 (2)
         else STM32F746.UInt2 (0));
   ...
   begin
      ...
      Dev.Periph.TDR.TDR := STM32F746.UInt9 (B);
      Accepted := True;
   end Tx_Push;
